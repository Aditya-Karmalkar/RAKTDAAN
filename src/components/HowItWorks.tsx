...
// ✅ Local Button Component (no external import)
function Button({
  children,
  variant = "primary",
  className = "",
  ...
  return (
    <button
      className={`h-11 px-6 text-base ${base} ${variants[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>
...
function StepCard({ step }: { step: Step }) {
  const circle = step.color === "red" ? "bg-red-600" : "bg-blue-600";
  const ring = step.color === "red" ? "ring-red-100" : "ring-blue-100";
...
function FeatureCard({ icon: Icon, title, desc }: { icon: any; title: string; desc: string }) {
  return (
    <motion.div
      variants={item}
      className="group p-6 bg-white rounded-2xl ring-1 ring-gray-100 shadow-sm hover:shadow-lg transition-all hover:-translate-y-1"
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="mb-20">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-2">For Donors</h2>
        ...
        <section className="mb-20">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-2">For Hospitals</h2>
        ...
        <section className="bg-white rounded-2xl p-8 md:p-12 ring-1 ring-gray-100 shadow-sm">
          ...
          <div className="mt-10 flex flex-col md:flex-row items-center justify-center gap-3">
            ...
            <Button variant="secondary">Register Hospital</Button>
          </div>
        </section>
      </div>
    </div>
